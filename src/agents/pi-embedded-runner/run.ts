    ...
    return enqueueGlobal(async () => {
      ...
      try {
        ...
        while (true) {
          ...
          // Feed the computed next-round thinking level back into the loop variable.
          // This overrides the static default for all rounds after the first,
          // enabling the model to get deeper reasoning when the session grows
          // complex (long history, many tools, truncation, or error recovery).
          if (nextThinkLevel !== undefined && nextThinkLevel !== thinkLevel) {
            thinkLevel = nextThinkLevel;
            authController.setThinkLevel(thinkLevel);
          }
          bootstrapPromptWarningSignaturesSeen =

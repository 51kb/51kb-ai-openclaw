...
export async function runEmbeddedAttempt(
  ...
  try {
    ...
    try {
      ...
      // Compute the next-round thinking level by analyzing session-level signals.
      // This runs even when aborted/timedOut so the caller always has a sane
      // nextThinkLevel to feed into the authController for the next attempt.
      const { resolveLargestSupportedThinkingLevel } = await import(
        "../../../auto-reply/thinking.js"
      );
      const { resolveAdaptiveThinkLevel } = await import("../../../agents/thinking-adaptive.js");
      const postRoundMessageCount = activeSession.messages.length;
      const postRoundAssistantCount = activeSession.messages.filter(
        (m: AgentMessage) => m.role === "assistant",
      ).length;
      const postRoundError =
        Boolean(lastAssistant?.errorMessage) || Boolean(getLastToolError?.());
      const modelMaxLevel = resolveLargestSupportedThinkingLevel(
        params.provider,
        params.modelId,
      );
      const nextThinkLevel = resolveAdaptiveThinkLevel(
        {
          promptChars: effectivePrompt?.length ?? 0,
          messageCount: postRoundMessageCount,
          assistantTurnCount: postRoundAssistantCount,
          toolCount: tools.length,
          bootstrapAnalysis: bootstrapAnalysis,
          hasErrorRecovery: postRoundError,
          sessionRound: postRoundAssistantCount,
        },
        params.thinkLevel,
        modelMaxLevel,
      );
      ...
      return {
        ...
        // Adaptive thinking: recommended level for the next round.
        // Present even on abort / error so the caller always has a baseline.
        nextThinkLevel,
      };
    } finally {
